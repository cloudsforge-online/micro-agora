...
export function registerServiceMetrics(metrics: Metrics): Metrics {
  return metrics
    ...
    .register({
      ...
      help: 'Freezes currently in force, per estate. Sampled at scrape time.',
      kind: 'gauge',
      // ── `network`, ADDED BY WAVE M5a, AND IT IS NOT DECORATION ─────────────────────────────
      //
      // This deployment holds a pool per network. Sampled without the label, the two planes write
      // the IDENTICAL series and the second `set` REPLACES the first — so a mainnet estate with
      // forty live freezes reads as testnet's zero, and the graph says the freeze machinery is
      // idle. An undeclared label is filtered by the registry and reported as a dropped write, so
      // this had to be widened rather than passed and hoped for.
      //
      // It stays on the REGISTRY rather than the module's labelled view: `policy_freezes_active`
      // collides with no other module's name, and stamping `module` on it would make it a
      // different series from the one dashboards already name.
      labels: ['network'],
    })
}
 ...
 * same route table is mounted beside four others.
 * ═══════════════════════════════════════════════════════════════════════════════════════════════
 */
 ...
 *
 * `/livez`, `/readyz` and `/metrics` exist on all five modules. Matching is FIRST-WINS, so a
 * second copy of each would be unreachable — a dead health endpoint that looks exactly like a live
 * one. agora's win, because it is the host and because Prometheus scrapes this target under
 * agora's job. `/readyz` still reports this module: it contributes probes to the host's one
 * `Lifecycle`, which is what makes one endpoint honest for five databases.
 */
 ...
 * reads the SAME estate-wide secret, which `mergedupstreams.test.ts` is what keeps true. Here it
 * would mean one of three keys silently deciding for the other two: a delivery devplatform signed
 * would be refused, or worse, a delivery signed with the estate-wide key would be accepted as
 * devplatform's when devplatform's own listener would have refused it.
 *
 ...
 * and the bare path answers 410 naming the split, so a subscription row nobody re-pointed is a
 * LOUD failure recorded in the producer's `outbox_deliveries.last_error` rather than an erasure
 * that quietly reaches one database out of three.
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 */
export const MOUNTED_EVENTS_PATH = '/v1/events/policy'
