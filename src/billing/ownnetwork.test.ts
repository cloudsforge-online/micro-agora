 ...
 * process as `src/billing/module.ts`; there is no `index.ts` in this directory any more, and the
 * merged process's own `../index.ts` builds several modules rather than this one.
 *
 ...
 * pod's database under the other estate's name while every other module was correct. A single
