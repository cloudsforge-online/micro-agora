...
import { BILLING_GRANT_SCOPE, BILLING_READ_SCOPE } from './server.ts'
...
test('the scopes this service uses are the ones in the contracts registry', () => {
  ...
  assert.ok(Object.hasOwn(SCOPES, BILLING_READ_SCOPE))
  assert.ok(Object.hasOwn(SCOPES, BILLING_GRANT_SCOPE))
  assert.equal(SCOPES[BILLING_READ_SCOPE].service, 'billing')
  assert.equal(SCOPES[BILLING_GRANT_SCOPE].service, 'billing')
})
