...
export const BILLING_GRANT_SCOPE = 'billing:grant'
...
function buildRoutes(): Route[] {
  ...
  return [
    ...
    define('POST', '/purchases', async (ctx, deps) => {
      ...
      if (principal.kind === 'service') requireScope(principal, BILLING_GRANT_SCOPE)
    ...
    define('GET', '/entitlements', async (ctx, deps) => {
      ...
      if (principal.kind === 'service') requireScope(principal, BILLING_READ_SCOPE)
    ...
    define('GET', '/internal/entitlements/:userId', async (ctx, deps) => {
      ...
      if (principal.kind !== 'service') {
        throw new ForbiddenError(`${BILLING_READ_SCOPE} (service token required)`)
      }
      requireScope(principal, BILLING_READ_SCOPE)
    ...
    define('POST', '/entitlements/:id/revoke', async (ctx, deps) => {
      const principal = await authenticate(ctx, deps)
      if (principal.kind === 'service') requireScope(principal, BILLING_GRANT_SCOPE)
      else if (!isAdmin(principal)) throw new ForbiddenError(`${BILLING_GRANT_SCOPE} or role:admin`)
 ...
 * same route table is mounted beside the platform host's and four other modules'.
 * ═══════════════════════════════════════════════════════════════════════════════════════════════
 */
 ...
 * agora's win, because it is the host and because Prometheus scrapes this target under agora's
 * job. `/readyz` still reports this module: it contributes probes to the host's one `Lifecycle`,
 * which is what makes one endpoint honest for every merged database.
 */
export const UNMOUNTED: ReadonlySet<string> = new Set(OPERATIONAL_ROUTES)

/** The erasure webhook path this module serves STANDALONE. Several modules mount this exact path. */
export const EVENTS_PATH = '/v1/events'

/**
 * The erasure webhook path this module serves INSIDE the merged process — and why it is not the
 * bare one.
 *
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 * **MORE THAN ONE MODULE MOUNTS `POST /v1/events`, AND THEY DO NOT VERIFY WITH THE SAME KEY.**
 *
 * agora verifies against `OUTBOX_SIGNING_SECRET`. policy verifies against `OUTBOX_ACCEPT_SECRETS`
 * falling back to `OUTBOX_SIGNING_SECRET`. billing verifies against its own `OUTBOX_ACCEPT_SECRETS`
 * (falling back to `[OUTBOX_SIGNING_SECRET]`) over the RAW body, and answers 403 rather than 401 on
 * a bad signature. devplatform verifies against a variable of its own, required at boot.
 *
 ...
 * reads the SAME estate-wide secret. Here it would mean one of several keys silently deciding for
 * the others: a delivery this module signed would be refused, or worse, a delivery signed with a
 * different key would be accepted as billing's when billing's own listener would have refused it.
 *
 ...
 * the bare path answers elsewhere, so a subscription row nobody re-pointed is a LOUD failure
 * recorded in the producer's `outbox_deliveries.last_error` rather than an erasure that quietly
 * reaches one database out of several.
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 */
export const MOUNTED_EVENTS_PATH = '/v1/events/billing'
 ...
 *      to change.
 *   2. **The handler closes over THIS module's deps, rebuilt against the request's handle.**
 *      `forRequest` is what the standalone listener did on every dispatch — it re-points
 *      `purchases` (which carries a database handle NEXT TO a ledger client) at `ctx.sql`, so a
 *      purchase written through the wrong handle cannot also post to the other estate's ledger.
 *      `RouteSpec.handle` takes only `ctx`, so there is no parameter through which the host's queue,
 *      verifier or stores could arrive.
 *   3. **`POST /v1/events` is re-pathed** to `MOUNTED_EVENTS_PATH` — see that constant's header.
 */
  ...
  return buildRoutes()
    ...
    .map((route) => ({
      ...
      handle: async (ctx: KernelContext<Db>): Promise<Reply> =>
        await handle(route, ctx, forRequest(deps, ctx.sql)),
    }))
}
