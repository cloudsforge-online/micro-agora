...
/** Every table this service owns. Order does not matter because CASCADE is used. Derived from the
 * single `TABLES` list in `./migrations.ts` (wave M5a) so the truncate list and the merged-process
 * collision matrix cannot drift apart. */
