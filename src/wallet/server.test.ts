...
import { WALLET_MONEY_SCOPE, WALLET_READ_SCOPE, WALLET_WRITE_SCOPE, createServer, registerServiceMetrics } from './server.ts'
...
test('a service token needs the scope for the authority it is using', { skip }, async () => {
  const reader = await sign({ sub: 'service:hub-api', scopes: [WALLET_READ_SCOPE] })
  await withServer({}, async (rig) => {
    ...
    assert.match(((await spend.json()) as { error: { message: string } }).error.message, new RegExp(WALLET_MONEY_SCOPE))
  })
})
...
test('THE RULE: the desk routes take an admin role, and no service scope substitutes', { skip }, async () => {
   ...
   */
  const service = await sign({ sub: 'service:hub-api', scopes: [WALLET_READ_SCOPE, WALLET_MONEY_SCOPE] })
  await withServer({}, async (rig) => {
...
test('the write scope is what a wallet registration needs', { skip }, async () => {
  const writer = await sign({ sub: 'service:hub-api', scopes: [WALLET_WRITE_SCOPE] })
  await withServer({}, async (rig) => {
    const res = await fetch(`${rig.url}/v1/wallets`, {
      method: 'POST',
