 ...
 * process as `src/wallet/module.ts`; there is no `index.ts` in this directory any more, and the
 * merged process's own `../index.ts` builds nineteen modules rather than this one.
 *
 * REPOINTED RATHER THAN DELETED, and rather than left pointing at the merged root, because the
 * defect this file catches is per MODULE: each module keys its own per-network map and its own
 * schema assertions, and a module that hardcoded `mainnet` would hold a testnet pod's database
 * under the other estate's name while eighteen others were correct. A single check against the
 * merged root would prove nothing about any of them — and in THIS module the consequence is a
 * user being shown another estate's money.
 *
