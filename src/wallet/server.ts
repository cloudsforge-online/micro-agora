 ...
 */
export const WALLET_READ_SCOPE = 'wallet:read'
export const WALLET_WRITE_SCOPE = 'wallet:write'
export const WALLET_MONEY_SCOPE = 'wallet:money'
...
function buildRoutes(): Route[] {
  return [
    ...
    route('GET', '/v1/wallets', async (ctx, deps) => {
      const principal = await authenticate(ctx, deps, WALLET_READ_SCOPE)
    ...
    route('POST', '/v1/wallets', async (ctx, deps) => {
      const principal = await authenticate(ctx, deps, WALLET_WRITE_SCOPE)
    ...
    route('GET', '/v1/deposits/assets', async (ctx, deps) => {
      await authenticate(ctx, deps, WALLET_READ_SCOPE)
    ...
    route('POST', '/v1/withdrawals', async (ctx, deps) => {
      const principal = await authenticate(ctx, deps, WALLET_MONEY_SCOPE)
      const body = await readJson(ctx.req)
      const userId = subjectUserId(principal, optionalString(body, 'userId'))
     ...
     *
     * `WALLET_MONEY_SCOPE` rather than `WALLET_READ_SCOPE`, and it is a POST rather than a GET, for the same
    ...
    route('POST', '/v1/conversions/quote', async (ctx, deps) => {
      await authenticate(ctx, deps, WALLET_MONEY_SCOPE)
      const body = await readJson(ctx.req)
      const done = deps.lifecycle.track()
      try {
...
/**
 * The webhook path this module serves, MOUNTED AND BARE — the process's third event family.
 *
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 * **`/events`, UNVERSIONED, AND THAT IS WHY IT NEEDS NO SUFFIX.**
 *
 * The merged process has three disjoint inbound-event families and this is the whole of the third:
 *
 *   `/v1/events/*`  ten modules, each with its own key, and a bare `/v1/events` answering 410
 *   `/ingest/*`     three modules, each with its own key, and a bare `/ingest` answering 410
 *   `/events`       THIS, and nothing else in the process declares it
 *
 * A suffix exists to stop two modules' keys deciding for each other at one shared path. Nobody
 * else mounts `/events`, so there is no second key and nothing to decide: suffixing it would move
 * a live inbox to a new address for no reason, and every producer's `event_subscriptions` row and
 * every `outbox_deliveries` retry would have to be re-pointed to buy nothing.
 *
 * It is therefore **deliberately absent from `../server.ts`'s `SPLIT_EVENT_PATHS`**, which lists
 * only the `/v1/events` family — putting it there would name it in the bare `/v1/events` 410 body
 * and send a producer at a path that answers with a different verifier. `../mergedroutes.test.ts`
 * asserts both halves: that this path is served exactly once in the process, and that it appears
 * in neither of the other two families' lists.
 *
 * The unversioned spelling is a fact about this service's history rather than a decision made
 * here, and it is left alone for the same reason the whole surface is: wallet is the one module of
 * this process whose paths CANNOT move. See `mountableRoutes`.
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 */
 ...
 *      write SUCCEEDS and reports nothing. In THIS module a wrong handle credits a testnet deposit
 *      to a mainnet balance, and the wallet is where a user looks to find out what they own.
 *   2. **The handler closes over THIS module's deps, rebuilt for THIS request.** `RouteSpec.handle`
 *      takes only `ctx`, so there is no parameter through which the host's stores could arrive.
 *      `forRequest(deps, ctx.network, ctx.sql)` rebuilds ALL FOUR domain bundles against the handle
 *      the kernel resolved — deposits, withdrawals, money and portfolio together, because
 *      rebuilding one and not the others would leave a deposit credited in one estate and a
 *      balance read from the other.
 *   3. **NOTHING IS REMOUNTED.** Not one path moves, and that is the decision this module is
 *      unusual for.
 *
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 * **WHY WALLET KEEPS EVERY BARE PATH, WHERE HUB MOVED ITS WHOLE SURFACE.**
 *
 * The rule for a collision is: the PUBLIC owner — the one the gateway routes to — keeps the bare
 * path, and the internal module remounts. Wallet is the public owner of everything it collides on.
 *
 *   * `deploy/gateway/dynamic/public-api.yml`'s `cf-api-wallet` routes SEVEN prefixes on
 *     `api.<apex>` straight at it: `/v1/wallets`, `/v1/deposits`, `/v1/withdrawals`, `/v1/spend`,
 *     `/v1/transfers`, `/v1/conversions` and `/v1/portfolio`. Those are the SDK's published paths.
 *   * `estate-web.yml` gives it the whole of the `pay.<suffix>` host.
 *
 * So the six paths hub also declared are wallet's, and hub's are the ones under `/v1/hub`. The
 * asymmetry is not a preference between two modules — it is which of them a caller outside this
 * estate is already addressing.
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 */
export function mountableRoutes(deps: ServerDeps, sql: NetworkSql): readonly RouteSpec<Db>[] {
  return buildRoutes()
    ...
    .map((route) => ({
      ...
      //
      // `handle` here takes a MATCHED pair rather than a route, because this module's router
      // captures parameters into `ctx.params` itself. The kernel has already done that work, so
      // the params it resolved are passed through rather than recomputed — recomputing them
      // against this module's own matcher would be two routers disagreeing about one URL.
      handle: async (ctx: KernelContext<Db>): Promise<Reply> =>
        await handle(
          { route, params: { ...ctx.params } },
          ctx,
          forRequest(deps, ctx.network, ctx.sql),
        ),
    }))
}
