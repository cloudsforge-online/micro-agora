 ...
 * the five names differ, so the runs do not even serialise against each other.
 *
 * It is worse for THESE five modules than it was for M3's pair. `inbox` and `jobs` exist in ALL
 * FIVE of this process's schemas; `outbox`, `event_subscriptions` and `outbox_deliveries` exist in
 * four of the five. So one shared database is not "two ledgers that confuse each other"; it is two
 * `create table inbox` racing, and then the later modules' tables never created at all, with a
 * green migrator. `migratortargets.test.ts` MEASURES that overlap rather than asserting it.
 *
