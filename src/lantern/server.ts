 ...
 *
 * `ServerDeps`, `PrincipalVerifier` and `LANTERN_READ_SCOPE` are re-exported here because that is where
...
export { LANTERN_READ_SCOPE } from './routes.ts'
 ...
 *
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 * **THE THREE OPERATIONAL PATHS GO; THE FALLBACK STAYS; EVERYTHING KEPT IS STAMPED.**
 *
 *   * `/livez`, `/readyz` and `/metrics` are agora's in the merged process — the target Prometheus
 *     scrapes with agora's job and the endpoints the kubelet probes. A second copy is dead on
 *     arrival under first-wins matching, and a dead health endpoint looks exactly like a live one.
 *     Nothing is lost: `/metrics` renders the host's registry, which this module's views write
 *     into, so every `lantern_*` and `analytics_*` series is still on the merged page.
 *   * The `fallback` is KEPT, and it is the only one in the process. It is what turns an unknown
 *     `/ingest/*` from an allowlisted browser origin into a CORS-readable 404 naming the paths that
 *     do exist — the failure that cost this estate months of browser telemetry that never arrived
 *     and that nobody could see failing. Its last line is byte for byte the kernel's own bare 404,
 *     so the other fifteen modules' misses are unchanged; `../mergedroutes.test.ts` pins both.
 *   * Every surviving route is stamped with THIS module's selector, because sixteen schemas in this
 *     process own a `jobs` table and two of them own an `events` table with different columns. A
 *     handler handed the wrong handle does not fail — it reads the other module's rows.
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 */
export function mountableRoutes(deps: ServerDeps, sql: NetworkSql): readonly RouteSpec[] {
