...
import { ANALYTICS_ADMIN_SCOPE, ANALYTICS_READ_SCOPE, createServer, hasExactScope, scrapeRefresh, type PrincipalVerifier, type ServerDeps } from './server.ts'
import {
  TEST_PEPPER,
  migrateTestDb,
  openDb,
...
const PRINCIPALS: Readonly<Record<string, Principal>> = {
  reader: { kind: 'service', service: 'admin-api', scopes: [ANALYTICS_READ_SCOPE] },
  admin: { kind: 'service', service: 'admin-api', scopes: [ANALYTICS_ADMIN_SCOPE] },
...
describe('the HTTP surface', { skip }, () => {
  ...
  describe('the scope matcher is exact, and neither auth package is changed', () => {
    it('grants the scope it names', () => {
      assert.equal(hasExactScope(PRINCIPALS['reader']!, ANALYTICS_READ_SCOPE), true)
    })
    ...
    it('a wildcard grants NOTHING — the whole point of choosing exact', () => {
      ...
      assert.equal(hasExactScope(PRINCIPALS['wildcard']!, ANALYTICS_READ_SCOPE), false)
      assert.equal(hasExactScope(PRINCIPALS['wildcard']!, ANALYTICS_ADMIN_SCOPE), false)
      assert.equal(hasExactScope(PRINCIPALS['star']!, ANALYTICS_READ_SCOPE), false)
    })
