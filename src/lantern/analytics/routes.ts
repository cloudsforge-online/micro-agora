 ...
 */
import { ANALYTICS_ADMIN_SCOPE, ANALYTICS_READ_SCOPE, hasExactScope, type ServerDeps } from './server.ts'
  ...
  const routes: readonly RouteSpec<Db>[] = [
...

    {
      ...
      handle: async (ctx) => {
        ...
        requireExactScope(principal, ANALYTICS_ADMIN_SCOPE)
...
function requireReader(principal: Principal): void {
  ...
  requireExactScope(principal, ANALYTICS_READ_SCOPE)
}
