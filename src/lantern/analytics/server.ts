...
import type { NetworkSql as AnalyticsNetworkSql, Sql } from '@cloudsforge/db'
import { OPERATIONAL_ROUTES, mountRoutes, type RequestContext, type RouteSpec } from '../../kernel.ts'
 ...
 */
export const ANALYTICS_READ_SCOPE = 'analytics:read'
export const ANALYTICS_ADMIN_SCOPE = 'analytics:admin'
...
/** The delivery path this module serves STANDALONE, and inside lantern's own two-module process. */
export const INGEST_PATH = '/ingest'

/**
 * The delivery path this module serves inside AGORA's process — wave M5c.
 *
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 * **THREE MODULES IN THAT PROCESS INGEST SIGNED DELIVERIES, AND THEY DO NOT SHARE A KEY.**
 *
 * activity verifies against `ACTIVITY_INGEST_SECRETS`, notify against
 * `NOTIFY_INGEST_SIGNING_SECRET`, and this module against `ANALYTICS_DELIVERY_SECRETS`. Wave M2 had
 * already split the first two onto `/ingest/activity` and `/ingest/notify` and left the bare
 * `POST /ingest` answering 410 naming them — which is the path THIS module serves. So the collision
 * is not a design choice to make here: the bare path is already spoken for by a refusal, and a
 * module keeping it would turn that refusal into a silent acceptance under the wrong key.
 *
 * Verifying once at a shared mount and fanning out is not available, and would not be wanted:
 *
 *   * it makes each secret a credential for EVERY sink, so whoever holds the weakest or oldest of
 *     the three holds all three capabilities;
 *   * it destroys rotation independence — the three accept-LISTS exist precisely so a rotation is
 *     not a flag day;
 *   * an envelope carries a topic, not a destination, and the three consumed-topic sets overlap;
 *   * and the three handlers answer three different response contracts, so one mount picks a loser.
 *
 * `../../activity/routes.ts`'s `INGEST_PATHS` carries the same argument at length and its 410 body
 * names this path. Each module keeps its OWN inbox as well as its own key: `inbox` exists in
 * thirteen of that process's sixteen schemas with the same three columns, so a delivery accepted
 * into the wrong one SUCCEEDS and dedupes an event that database has never seen.
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 */
export const MOUNTED_INGEST_PATH = '/ingest/analytics'
 ...
 */
export function mountableRoutes(deps: ServerDeps, sql: AnalyticsNetworkSql): readonly RouteSpec<Sql>[] {
  return createRoutes(deps)
    .filter((spec) => !OPERATIONAL_ROUTES.has(spec.path))
    .map((spec) => ({
      method: spec.method,
      path: spec.path === INGEST_PATH ? MOUNTED_INGEST_PATH : spec.path,
      sql,
