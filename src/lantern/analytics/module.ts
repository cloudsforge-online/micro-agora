...
// `mountableRoutes`, `INGEST_PATH` and `MOUNTED_INGEST_PATH` live in `./server.ts` rather than here
// since wave M5c, and the move is load-bearing: THIS file imports `./env.ts`, which validates the
// pepper at import and calls `process.exit(1)` on an incomplete configuration. A structural test
// that only wants to read the route TABLE — `../../mergedroutes.test.ts`, which needs no database
// and no secrets — would otherwise kill the test runner just by importing it.
import {
  INGEST_PATH,
  MOUNTED_INGEST_PATH,
  mountableRoutes,
...
// Re-exported from `./server.ts`, which is where they moved so a structural test can read them
// without importing `./env.ts`. Named here too because this is the file the host talks to.
export { INGEST_PATH, MOUNTED_INGEST_PATH } from './server.ts'
