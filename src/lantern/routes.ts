...
export const LANTERN_READ_SCOPE = 'lantern:read'
...
async function authorise(ctx: RequestContext, deps: ServerDeps): Promise<Principal> {
  ...
  if (presented && constantTimeEquals(presented, deps.token)) {
    return { kind: 'service', service: 'lantern-token', scopes: [LANTERN_READ_SCOPE] }
  }
  ...
  if (principal.scopes.includes(LANTERN_READ_SCOPE)) return principal
  throw new ForbiddenError(LANTERN_READ_SCOPE)
}
