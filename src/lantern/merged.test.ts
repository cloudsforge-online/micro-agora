...
const { MOUNTED_INGEST_PATH, createAnalyticsModule } = await import('./analytics/module.ts')
...
describe('the merged surface', { skip }, () => {
  ...
  describe('the two route tables are mounted, and neither shadows the other', () => {
    ...
    it('keeps analytics’ signed inbox and lantern’s browser sink apart, on their own paths', async () => {
      /*
       * ════════════════════════════════════════════════════════════════════════════════════════
       * **WAVE M5c MOVED ANALYTICS' INBOX, AND THIS CASE IS WHERE THAT IS VISIBLE.**
       *
       * It used to be the bare `POST /ingest`. agora's process — which this module now lives in —
       * already served that path as a 410 naming `/ingest/activity` and `/ingest/notify`, because
       * activity and notify verify with different secrets and wave M2 had split them. A third
       * module keeping the bare path would have turned that deliberate refusal into a silent
       * acceptance under a key the caller did not sign with.
       *
       * So `mountableRoutes` remounts it at `MOUNTED_INGEST_PATH`, and the property this case
       * asserts is unchanged in strength: each ingest path reaches EXACTLY ONE module, and a
       * delivery cannot land in the wrong one. What changed is which string that is.
       * ════════════════════════════════════════════════════════════════════════════════════════
       */
      const signed = await fetch(`${url}${MOUNTED_INGEST_PATH}`, { method: 'POST', body: '{}' })
      ...
      assert.equal(((await signed.json()) as { error: { code: string } }).error.code, 'bad_signature')
      ...
      // And nothing answers the BARE path here. In agora's process activity's 410 does, naming all
      // three signed inboxes; in this two-module composition there is no owner for it at all, which
      // is the honest answer and — crucially — is NOT analytics accepting an unsigned body.
      const bare = await fetch(`${url}/ingest`, { method: 'POST', body: '{}' })
      assert.equal(bare.status, 404, 'analytics must no longer answer the bare path')
      assert.equal(((await bare.json()) as { error: { code: string } }).error.code, 'not_found')
    })
