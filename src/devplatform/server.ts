...
export function registerServiceMetrics(metrics: Metrics): Metrics {
  return metrics
    .register({
      ...
      kind: 'gauge',
      // `network`, added by wave M5a: this deployment holds a pool per estate, and sampled without
      // the label the second plane's `set` REPLACES the first's. An undeclared label is filtered by
      // the registry and reported as a dropped write, so the spec is widened rather than the label
      // passed and hoped for.
      labels: ['network'],
    })
    .register({
...
export function scrapeRefresh(deps: {
  readonly sql: Db
  readonly metrics: Metrics
  /** Wave M5a: `{ network }` in the merged process, where two planes write this gauge. */
  readonly labels?: Readonly<Record<string, string>>
}): () => Promise<void> {
  return async () => {
    deps.metrics.set('devplatform_up', 1, deps.labels ?? {})
  }
}
 ...
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 */
export const MOUNTED_EVENTS_PATH = '/v1/events/devplatform'
