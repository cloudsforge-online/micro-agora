 ...
 *
 * `labels` exists for wave M5a. In the merged process this module holds a queue per ESTATE and
 * shares a registry with four other modules, and `jobs_pending`/`jobs_overdue` carry no `kind` at
 * all — so without a `{ module, network }` pair every writer's sample OVERWRITES every other's on
 * each scrape, and a wedged queue is ABSENT from the graph rather than high. The standalone caller
 * passes nothing and behaves exactly as before.
 */
export async function sampleQueue(
  queue: JobQueue,
  metrics: Metrics,
  labels: Readonly<Record<string, string>> = {},
): Promise<void> {
  const stats = await queue.stats()
  metrics.set('jobs_pending', stats.pending, labels)
  metrics.set('jobs_overdue', stats.overdue, labels)
}

/** Pending webhook deliveries, by whether they are still retriable. Sampled at scrape time. */
export async function sampleDeliveries(
  sql: Db,
  metrics: Metrics,
  labels: Readonly<Record<string, string>> = {},
): Promise<void> {
  ...
  `
  const row = rows[0]
  metrics.set('devplatform_webhook_pending', row?.pending ?? 0, labels)
  ...
  metrics.set('devplatform_webhook_abandoned', row?.stuck ?? 0, labels)
}
