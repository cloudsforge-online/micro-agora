...
test('no source file starts an interval doing domain work', () => {
  ...
  // `module.ts`, not `index.ts` — wave M5a moved this service's composition root into agora's
  // process (micro-deploy `docs/service-merge-plan.md`). Repointed rather than dropped: the
  // composition root is where a `setInterval` is MOST likely to be written, because it is the file
  // that owns the process's timers, and the merged root would not be covered by this module's
  // suite at all. The merged root itself, and every other module's, is swept by
  // `../merged.test.ts`, which walks the whole of `src/` rather than a list of seven names.
  const files = ['server.ts', 'jobs.ts', 'module.ts', 'outbox.ts', 'webhooks.ts', 'quotas.ts', 'apikeys.ts']
