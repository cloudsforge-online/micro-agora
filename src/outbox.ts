 ...
 *
 * `agora.post.created` is what activity and notify read, and `agora.voice.renamed` is how the rest
 * of the estate learns that the handle in a link it already stored has changed. Neither moves
 * money, which makes it tempting to publish after the commit and be done — and that is exactly the
 * reasoning that produces a notification for a post that was rolled back, or silence for one that
 * was not.
 *
 * There is deliberately no `agora.voice.created`. `ensureVoice` runs inside the first write of
 * every route, so an event there would fire on a transaction that may still roll back for an
 * unrelated reason, and it would announce an account that has not yet said anything. The estate
 * learns about a voice when it posts. The fourteen topics in `@cloudsforge/contracts-events` are
 * the complete list, and `topics.test.ts` is what keeps this file from inventing a fifteenth.
 * A person told they were mentioned in a post that does not exist has no way to find out they were
 * not; a person never told is simply not part of the conversation. Both are unrecoverable after
 * the fact, and neither is worth the transaction this file avoids.
 */
 ...
 *
 *   const post = await withOutbox(sql, SERVICE, async (tx, emit) => {
 *     const row = await insertPost(tx, input)
 *     emit({ topic: 'agora.post.created', key: row.id, payload: { … } })
 *     return row
 *   })
 *
 ...
 *   - **`actor` falls back to `service:agora`.** An emit with no actor was this service acting on
...
export function buildEnvelope(row: OutboxRow): EventEnvelope {
  return {
    ...
    actor: row.actor ?? serviceActor('agora'),
